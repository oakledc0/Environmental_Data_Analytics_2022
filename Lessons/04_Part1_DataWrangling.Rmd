...
> Add notes here: to estimate lake productivity and the assortment of different processes that affects
...
```{r, results = "hide"}
vignette("dplyr") #displays need little summary of the package as well as 
#examples
```
...
# Did the methods arrive at the same result? YESYESYESYES
...
NTL.phy.data.TuesLake90to99 <- filter(NTL.phys.data, lakename == "Tuesday Lake",
...
#^NOTICE, that desc() reads in the descening order
#??????????????????? is this usuable outside of dplyr?

# Exercise: 
# Arrange NTL.phys.data by temperature, in descending order. 
# Which dates, lakes, and depths have the highest temperatures?
NTL.phys.data.temps.descending <- arrange(NTL.phys.data, desc(temperature_C))
head(NTL.phys.data.temps.descending)
```
### Select
...
#creates subset dataframe that includes only lakename column and columns 
#between (inclusive) sampledate and temperature_C
```
### Mutate
...
#this last one seems better since you don't need to know how many entries are
