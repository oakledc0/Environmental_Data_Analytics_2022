...
Rainfall_plot <- ggplot(Rainfall_BR_processed, aes(x = Date, y = Rainfall)) +
  geom_point() +
  geom_line() +
  ylab("Rainfall") +
...
Temperature_plot <- ggplot(Temp_BR_processed, aes(x = Date, y = Temperature_C)) +
  geom_point() +
  geom_line() +
  ylab("Temperature") +
  geom_smooth(method = "lm")
...
temperature_ts <- ts(Temp_BR_processed$Temperature_C, start = c(1901,1), frequency = 12)

#or, find the first year and month of your data and then specify
# fmonth <- month(first(Temp_BR_processed))
# fyear <- year(first(Temp_BR_processed))
# 
# temperature_ts <- ts(Temp_BR_processed, start = c(fmonth, fyear), frequency = 12)
...
rainfallTS_decomp <- stl(rainfall_ts, s.window = "periodic")
plot(rainfallTS_decomp)

tempTS_decomp <- stl(temperature_ts, s.window = "periodic")
plot(tempTS_decomp)
...
rainfallTS_components <-
  as.data.frame((rainfallTS_decomp$time.series[,1:3]))


tempTS_components <-
  as.data.frame((tempTS_decomp$time.series[,1:3]))

rainfallTS_components <-
  mutate(rainfallTS_components, Observed = Rainfall_BR_processed$Rainfall,
         Date = Rainfall_BR_processed$Date)

tempTS_components <-
  mutate(tempTS_components, Observed = Temp_BR_processed$Temperature_C,
         Date = Temp_BR_processed$Date)


# Exercise 10: Visualize how the trend maps onto the data for both series

tempPlot <- ggplot(tempTS_components) +
  geom_line(aes(x = Date, y = Observed)) +
  geom_line(aes(x = Date, y = trend, color = "red")) +
  labs(y = "Temperature (C)", color = "")
print(tempPlot)
