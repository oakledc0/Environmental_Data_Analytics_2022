...
> Answer: Mean AQI is the same as the threshold value
...
```{r}
#Exercise 2:
ex2_P1 <- ggplot(EPAair) +
  geom_density(aes(x = Ozone, fill = as.factor(Year)), alpha = 0.5) +
  labs(y = "Density", x = "Ozone AQI Value", fill = "Year") +
  scale_fill_manual(values = c("blue", "yellow"))
ex2_P1

library(viridis)
ex2_P2 <- ggplot(EPAair, aes(x = as.factor(Year), y = Ozone)) +
  geom_boxplot() +
  labs(y = "Ozone AQI Value", x = "") +
  stat_summary(geom = "text", fun = max, vjust = -0.5, label = c("a", "b"),
               color = "red")
ex2_P2

ex2_P3 <- ggplot(EPAair, aes(x = as.factor(Year), y = Ozone)) +
...
ex3_lm1 <- lm(EPAair$PM2.5 ~ EPAair$Ozone)
summary(ex3_lm1)

#Exercise 4: Build a scatterplot. Add a line and standard error for the linear regression. 
ex4_P1 <- ggplot(EPAair, aes(x = Ozone, y = PM2.5)) +
  geom_point() +
  geom_smooth(method = lm) +
  labs(x = "Ozone AQI Value", y = "PM2.5 AQI Value", title = "Ozone vs PM2.5" ) +
  theme(title = element_text(hjust = -500))
ex4_P1
...
#create correlation for all variables (Latitude to PM2.5)
the_cor <- cor(EPAair %>% select(SITE_LATITUDE:PM2.5) %>% drop_na())

#create corrplots to seeeeeeeee which correlations are strong
...
ex6_lm1 <- lm(data = EPAair, PM2.5 ~ SITE_LATITUDE + SITE_LONGITUDE + Month +
                Year + Ozone)
summary(ex6_lm1)
