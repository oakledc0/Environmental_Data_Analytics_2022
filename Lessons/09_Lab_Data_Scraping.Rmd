...
#Get the data, which starts on line 30
gage_data2 <- read.table(theURL,skip = 29,header=TRUE,sep='\t') 
...
```{r}
# #Apply the API key
# census_api_key("39165430be07fa5d31f42f2f9ac29bd7d56e84b7", install = TRUE,
#                overwrite = TRUE)
...
df_power <- data.frame("StateName" = the_states,
                       'AvgPrice' = the_price,
                       'NetSumCapacity' = the_capacity,
                       'NetGenrtn' = net_generation,
...
the_years <- c(2017,2018,2019)

df2 <- map(the_years, scrape.it) %>% bind_rows()
```
