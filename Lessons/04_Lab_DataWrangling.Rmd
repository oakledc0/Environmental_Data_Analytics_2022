...
NTL.phys.data.90sTues <- filter(NTL.phys.data, year4 %in% 1990:1999, lakename == "Tuesday Lake")


```
Question: Why don't we filter using row numbers?

> Answer: Because they are arbitrary and can be messy
...
library(plyr)
library(dplyr)
NTL.phys.data.meprocess <-
  NTL.phys.data %>%
  separate(sampledate, c("Y", "m", "d")) %>%
  filter(Y %in% 1990:1999, m == "07", lakename == "Tuesday Lake")
...
NTL.phys.data.meprocess2 <- 
  NTL.phys.data.meprocess %>% 
  filter(depth == 0) %>% 
  summarise(surfTemp = mean(temperature_C))
NTL.phys.data.meprocess2  
```
