...
ggplot(huc8_sf) + geom_sf(color = "black",fill = "purple")
...
huc8_sf_filt <- st_read('./Data/Spatial/NCHUC8.shp') %>% 
  filter(SUBBASIN == 'Upper Neuse')

#Create a map
ggplot() + 
  geom_sf(data = huc8_sf, fill = "orange") +
  geom_sf(data = huc8_sf_filt, fill = "purple")
...
ENVatlas_WindEng <-
  ...
  group_by(HUC_8) %>% 
  summarise(SumEng_HUC8 = sum(AvgWindEnergy))
  

#Join to HUC_8 features
library(dplyr)
huc8_sf_wind <- left_join(huc8_sf, ENVatlas_WindEng,'HUC_8')


#View the outputs
ggplot() +
  geom_sf(data = huc8_sf_wind, aes(fill = SumEng_HUC8)) +
  labs(fill = "MW/year")
...
triangle_co <- counties_sf_utm %>% 
  filter(NAME %in% c('Durham','Chatham', 'Orange', 'Wake'))

  
#Grab the intersecting HUC_8s
traingle_HUC8s <- counties_sf_utm[triangle_co,]


#Intersect the HUC_8s
triangle_HUC8_co <- triangle_co %>% 
  st_intersection(counties_sf_utm)

mapview(triangle_HUC8_co, alpha.regions = 0)
